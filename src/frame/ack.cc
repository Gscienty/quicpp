...
quicpp::frame::ack::ack(std::basic_istream<uint8_t> &in) {
    ...
    while (len--) {
        ...
        top -= uint64_t(gap);
        this->ranges.push_back(std::make_pair(top - uint64_t(len), top));
        top -= uint64_t(len);
    }
}
...
size_t quicpp::frame::ack::size() const {
    size_t result = 1 +
        ...
        quicpp::base::varint(this->delay.count()).size() +
        quicpp::base::varint(this->ranges.size() - 1).size();
    
    auto begin = this->ranges.begin();
    ...
    begin++;
    std::for_each(begin, this->ranges.end(),
...
uint64_t quicpp::frame::ack::largest() const {
    return std::get<quicpp::frame::ack_range_largest>(*this->ranges.begin());
}

uint64_t quicpp::frame::ack::smallest() const {
    return std::get<quicpp::frame::ack_range_smallest>(*this->ranges.rbegin());
}
