...
quicpp::stream::incoming_bidi_streamsmap::accept_stream() {
    ...
    while (true) {
        ...
        auto ret_str_itr = this->streams.find(this->next_stream);
        if (ret_str_itr != this->streams.end()) {
            ret_str = ret_str_itr->second;
            break;
        }
...
get_or_open_stream(quicpp::base::stream_id_t id) {
...

    {
        ...
        for (quicpp::base::stream_id_t new_id = start;
             new_id <= id;
             new_id += 4) {
...
void quicpp::stream::incoming_bidi_streamsmap::
